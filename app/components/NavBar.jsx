...
export default function NavBar(){
    ...
    return(
        <nav className={pathname != '/' ? 'flex flex-row justify-between px-4 bg-neutral-800 border-b border-neutral-700 w-full fixed z-40':'flex flex-row justify-between px-4 bg-transparent w-full fixed z-40'}>
            ...
            <section className='grid grid-cols-3 gap-4 place-items-center text-orange-500 font-semibold'>
                <Link href={'/'} 
                onClick={() => router.push('/')}
                className={pathname == '/' ? "hidden sm:block border-b-2 border-orange-500 duration-150": "hidden sm:block hover:-translate-y-0.5 hover:font-normal duration-300 font-light border-none"}>
                    Home
                </Link>

                <Link href={'/about'} 
                onClick={() => router.push('/about')}
                className={pathname == '/about' ? "hidden sm:block border-b-2 border-orange-500 duration-150": "hidden sm:block hover:-translate-y-0.5 hover:font-normal duration-300 font-light border-none"}>
                    About
                </Link>

                <Link href={'/contact'} 
                onClick={() => router.push('/contact')}
                className={pathname == '/contact' ? "hidden sm:block border-b-2 border-orange-500 duration-150": "hidden sm:block hover:-translate-y-0.5 hover:font-normal duration-300 font-light border-none"}>
                    Contact
                </Link>
            </section>
            ...
            <button onClick={()=>setIsOpen(!isOpen)} 
                className="flex flex-col sm:hidden justify-center items-center ">
                    <span className={`z-50 bg-orange-500 block transition-all duration-300 ease-out 
                                    h-0.5 w-6 rounded-sm ${isOpen ? 
                                    'rotate-45 translate-y-1' : '-translate-y-0.5'
                                    }`} >
                    </span>

                    <span className={`z-50 bg-orange-500 block transition-all duration-300 ease-out h-0.5 w-6 rounded-sm my-0.5 ${isOpen ? 'opacity-0' : 'opacity-100'}`}>
                    </span>
